...
class RegiUPRApp(QStackedWidget):
    def __init__(self):
        ...
        if self.main_menu_page is not None:
    ...
    def show_main_menu(self):
        student_id = self.login_page.get_student_id()  # Get the student ID from the login page
        if self.main_menu_page is None:
            self.main_menu_page = MainMenu(student_id)  # Initialize MainMenu with student_id
